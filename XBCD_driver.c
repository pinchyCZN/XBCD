...
NTSTATUS XBCDAddDevice(IN PDRIVER_OBJECT pDriverObject, IN PDEVICE_OBJECT pDeviceObject)
{
    NTSTATUS status = STATUS_SUCCESS;
    PDEVICE_EXTENSION pDevExt = GET_MINIDRIVER_DEVICE_EXTENSION(pDeviceObject);

	KdPrint(("XBCDAddDevice - Entry\n"));

	PAGED_CODE();

	pDevExt->pPdo = GET_PHYSICAL_DEVICE_OBJECT(pDeviceObject);
	pDevExt->pDeviceObject = pDeviceObject;
	pDevExt->DriverObject = pDriverObject;
    ...
    pDeviceObject->Flags |= DO_POWER_PAGABLE | DO_DIRECT_IO;

	pDevExt->pLowerPdo = GET_LOWER_DEVICE_OBJECT(pDeviceObject);
...
NTSTATUS CreateChild(PDEVICE_EXTENSION pdx, ULONG child_num, PDEVICE_OBJECT* ppdo)
{
	NTSTATUS status;
	...
	if (!NT_SUCCESS(status))
	{
		KdPrint(("IoCreateDevice (child) failed - %X\n", status));
		return status;
	}
